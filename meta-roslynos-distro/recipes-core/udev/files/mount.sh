...
#
AUTOCALLSCRIPTS="1"
DIRECTORYCALLSCRIPTS="roslynos-init"
...
automount_systemd() {
    ...
    *)
    ...
    then
        ...
        if [ $AUTOCALLSCRIPTS = "1" ]; then
			DIR="/run/media/$name/$DIRECTORYCALLSCRIPTS"
			for SCRIPT in $DIR/*
			do
				if [ -f $SCRIPT -a -x $SCRIPT ]
				then
					logger "$SCRIPT script found on removable media, execute..."
